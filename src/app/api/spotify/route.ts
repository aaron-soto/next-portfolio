...
const base = Buffer.from(`${process.env.SPOTIFY_CLIENT_ID}:${process.env.SPOTIFY_CLIENT_SECRET}`).toString('base64');
...
export async function GET(req: NextRequest) {
  ...
  const data = {
    ...
    artist: responseData.item.album.artists.map((artist: any) => artist.name).join(', '),
